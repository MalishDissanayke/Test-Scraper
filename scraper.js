...
async function scrapeMatches() {
  ...
  while (attempt < maxAttempts && !success) {
    ...
    try {
      ...
      success = true; // If no errors, set success to true to exit loop
      ...
      if (attempt < maxAttempts) {
        ...
        await new Promise(res => setTimeout(res, 5000)); // Sleep before retrying
      }
    } finally {
      await browser.close();
    }
  ...
  if (!success) {
    ...
    process.exit(1); // Exit with failure code if all attempts failed
  }
}
